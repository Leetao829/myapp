...
const RedisApply: React.FC = () => {
  ...
  const handleMenuClick = (e: { key: React.Key }) => {
    history.push(`/resource/apply/redis/${e.key}`);
  };
  
  // 如果当前路径是/resource/apply/redis，重定向到/resource/apply/redis/user
  useEffect(() => {
    if (currentPath === '/resource/apply/redis') {
      history.push('/resource/apply/redis/user');
    }
  }, [currentPath]);
